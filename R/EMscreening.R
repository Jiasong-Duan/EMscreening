...
##two datasets, Phenotype, DNAm

EMscreening=function(Phenotype,DNAm,Continuous=TRUE,Cutoff.Joint=0.1,Iterations=100,Train.Pvalue=0.05,Test.Pvalue=0.05,Percentage=0.5){
...
for (i in 1:p){
  tempdata <- cbind(X=matrix(Phenotype[,1],ncol=1),Y=matrix(Phenotype[,2],ncol=1), M=matrix(DNAmdata[i, ],ncol=1))
...
for (i in 1:p){
  if (joint.out.all[i] >=Cutoff.Joint) {
    ...
    temp<- cbind(X=matrix(Phenotype[,1],ncol=1),Y=matrix(Phenotype[,2],ncol=1), M=matrix(DNAmdata[i, ],ncol=1))
...
if (Continuous==TRUE) {
   ...
   Path.out.test=glm_indirect_jointest(test)
      ...
      sig_path[k] <- ifelse (Path.out.train<Train.Pvalue &Path.out.test<Test.Pvalue,1,0)
    }
